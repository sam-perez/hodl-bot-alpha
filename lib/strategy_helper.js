...
module.exports = {
  ...
  getBuyAmountInDollarsForConfig: function({ buyConfig, amountSpent, lastPurchaseDate }) {
    ...
    const amountLeftToBuy =
            (Math.round(100 * amountInvested) - Math.round(100 * amountSpent)) / 100;
    ...
    if (theoreticalAmountToBuy >= MIN_BUY_AMOUNT_IN_DOLLARS) {
      ...
      if (amountLeftOverAfterThisBuy < MIN_BUY_AMOUNT_IN_DOLLARS) {
        ...
        // it would appear that dividing a JS integer by 100 will not have any rounding error for reasonable numbers (up to 100000000)
        amountToBuyInDollars = Math.round(100 * theoreticalAmountToBuy) / 100;
      }
    } else {
